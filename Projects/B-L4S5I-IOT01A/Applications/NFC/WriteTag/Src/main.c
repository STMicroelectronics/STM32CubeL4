...
char Message[] = {"This SMS was generated automatically by tapping your phone near MB1297\0"};
char Instruction[] = {"This is an example of NFC generated SMS using MB1297 board\0"};
...
char Subject[] = {"ST25DV-I2C S/N 754FHFGJF46G329 WARRANTY\0"};
...
static void URIwrite_demo(void)
{
  ...
  /* First write NDEF */
  while (NDEF_WriteURI(&URI) != SUCCESS);
}  
...
//  memcpy(pSMSStruct->Information, Instruction, strlen(Instruction)+1);  
  ...
  status = NDEF_WriteSMS ( pSMSStruct );
...
static void Vcardwrite_demo(void)
{
  ...
  memset(&VcardStruct,0,sizeof(VcardStruct));

  pVcardStruct = &VcardStruct;
  
  memcpy(pVcardStruct->FirstName, FirstName, strlen(FirstName)+1);
  memcpy(pVcardStruct->Name, FirstName, strlen(FirstName)+1);
  ...
  status = NDEF_WriteVcard ( &VcardStruct );
  if(status != SUCCESS)
  {
    Error_Handler();    
  }
