...
interruption is generated, the LED1 toggles.
...
an interrupt is generated, LED1 toggles and the repetition counter is decremented.
...
Therefore, user can observe the LED toggling at a varying period (long LED1 toggling
followed by four short ones).

As described in the Reference Manual, writing to the repetition counter has no effect on
the content until the next repetition underflow event occurs. The repetition counter continues
to decrement each LPTIM counter overflow event and only when a repetition underflow event is
generated, the new value written into LPTIM_RCR REP[7:0] is loaded into the repetition counter.
This is why, as the very start, the first period is longer than the others (there are more
LED1 toggling occurrences).
