...
This example uses the DMA2D to transfer an internal SRAM static texture array
aBufferInput[] of size 16x32 and format ARGB4444 (16 bpp) to an internal-SRAM
destination buffer, in array aBufferResult[]. The number of bytes transferred
is 16x32x2=1024 bytes.
...
32L4P5GDISCOVERY board's LEDs can be used to monitor the transfer status:
 ...
 If everything all right, at the end of the test, LED1  must be ON, LED2 must be OFF.
