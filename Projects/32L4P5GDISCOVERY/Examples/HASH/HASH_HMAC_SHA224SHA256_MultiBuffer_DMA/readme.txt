...
For SHA-224, the same hashing operation is carried out twice for illustration purposes:
first uin polling mode, then in DMA mode.
...
STM32 board LED is used to monitor the example status:
  - LED1 is ON when correct digest values are calculated
  - LED2 is blinking (200 ms period) when an incorrect digest value is calculated or when there is an initialization error.
