/**
  ******************************************************************************
  * @file    Examples_LL/ADC/ADC_SingleConversion_Stop2_LP/Src/main.c
  * @author  MCD Application Team
  * @brief   This example describes how to use an ADC peripheral with a
  *          system configuration optimized for power consumption,
  *          to perform an acquisition at low frequency (between 1Hz and 1kHz).
  *          This examples uses low-power mode Stop2 and RTC.
  *          ADC conversion data are transferred by DMA into a table,
...
/** @addtogroup ADC_SingleConversion_Stop2_LP
  * @{
  */
...
/*       values definition.                                                   */
  ...
  /* Note: In this example, ADC clock is set to 1/2 of CPU clock. Delay is    */
  /*       optimized in function of this clock ratio.                         */
  #define ADC_DELAY_CALIB_ENABLE_CPU_CYCLES  (LL_ADC_DELAY_CALIB_ENABLE_ADC_CYCLES * 2)
...
/* Definitions of data related to this example */
  ...
  #define ADC_CONVERTED_DATA_BUFFER_SIZE   ( 100U)

  /* Init variable out of expected ADC conversion data range */
  #define VAR_CONVERTED_DATA_INIT_VALUE    (__LL_ADC_DIGITAL_SCALE(LL_ADC_RESOLUTION_12B) + 1)

/* Definitions of ADC acquisition frequency */
  /* ADC acquisition frequency (unit: Hz).                                    */
  /* Range is min=1Hz, max=1kHz.                                              */
  #define ACQUISITION_FREQUENCY_HZ                       (10U)

/* Time base for RTC wake-up timer */
/* Note: Wake-up timer clock configured at 1kHz */
#define RTC_WUT_TIME_MS            (1000 / ACQUISITION_FREQUENCY_HZ)

/* Oscillator time-out values */
#define LSI_TIMEOUT_VALUE          ( 100U)  /* 100 ms */
#define RTC_TIMEOUT_VALUE          (1000U)  /* 1 s */
...
void     SystemClock_MSI_calibration(void);
void     Configure_DMA(void);
void     Configure_RTC(void);
void     Configure_ADC(void);
void     Activate_ADC(void);
void     DeActivate_ADC(void);
void     LowPowerModeInit(void);
void     AcquisitionInit(void);
void     AcquisitionStart(void);
void     AcquisitionStop(void);
void     AcquisitionPauseLowPower(void);
void     AcquisitionResumeLowPower(void);
void     LED_Init(void);
...
int main(void)
{
  ...
  /* Set system clock for low-power execution */
  SystemClock_Config_LowPower();

  /* Low-power mode initialization */
  LowPowerModeInit();

  /* Disable LSE propagation to all peripherals except RTC */
  LL_RCC_LSE_DisablePropagation();

  /* Optimize BOR and PVD monitoring while in Stop 2 mode */
  ...
  /* Acquisition pause before entering in low-power mode */
  AcquisitionPauseLowPower();
  ...
  while (1)
  {
    ...
    /* At this point: Acquisition paused, low-power mode Stop2.               */
    /*                Wake-up by RTC wake-up timer.                           */

    #if defined(OPTION_MONITOR_CPU_ACTIVITY)
    LED_Init();
    LED_On();
    #endif /* OPTION_MONITOR_CPU_ACTIVITY */

    /* Acquisition resume after exiting from low-power mode */
    AcquisitionResumeLowPower();

    /* Clear the RTC wake-up timer interrupt pending flag */
    LL_RTC_ClearFlag_WUT(RTC);

    /* Clear the EXTI flag for RTC wake-up timer */
    ...
    /* At this point: Acquisition on going, low-power mode sleep.             */
    /*                Wake-up by ADC group regular end of unitary conversion. */
...
#if defined(OPTION_THRESHOLD_DEBUG)    
    {
      static uint32_t i = 0; 
      
      if (aADCxConvertedData[i] > 2048) 
      {
            LED_Init();
            LED_On();
      }
    ...
    /* Note: ADC conversions are performed indefinitely (trigger from SW at   */
    /*       each wake-up by RTC: ADC conversions as long as timer is running)*/
...
#if defined(OPTION_PWR_OPTIM_BUFFER_PROCESSING)
...
void Configure_RTC(void)
{
  ...
  /* Set RTC clock source prescalers */
  /* Note: Configuration to have a RTC wake-up clock of 1kHz                  */
  /*       LSI frequency: 32kHz                                               */
  /*       RTC asynchronous prescaler: 32                                     */
  /*       RTC synchronous prescaler: not used by RTC wake-up timer           */
  LL_RTC_SetAsynchPrescaler(RTC, 32 - 1);
  LL_RTC_SetSynchPrescaler(RTC, 0);
  ...
  /* Setting RTC wake-up time */
  LL_RTC_WAKEUP_SetAutoReload(RTC, RTC_WUT_TIME_MS - 1U);
  LL_RTC_WAKEUP_SetClock(RTC,  LL_RTC_WAKEUPCLOCK_CKSPRE );

  /* Clear the wake-up timer interrupt pending bit */
  LL_RTC_ClearFlag_WUT(RTC);

  /* RTC wake-up timer Interrupt Configuration: EXTI configuration */
  LL_EXTI_EnableIT_0_31(LL_EXTI_LINE_20);
  LL_EXTI_EnableRisingTrig_0_31(LL_EXTI_LINE_20);

  /*## Configure the NVIC for RTC wake-up timer ##############################*/
  /* Note: NVIC not enabled since events are used instead of interruptions    */
  ...
  /* Note: RTC wake-up counter is enabled afterwards                          */
  /*       using function "LL_RTC_WAKEUP_Enable()".                           */
...
void Activate_ADC(void)
{
  ...
  if (LL_ADC_IsEnabled(ADC1) == 0)
  {
    ...
    /* Perform ADC calibration only once */
    /* Note: ADC calibration is performed only once in this example because   */
    /*       RCC ADC clock is not disabled.                                   */
    /*       In case of user application disable RCC ADC clock, ADC           */
    /*       calibration must be performed again or ADC calibration factors   */
    /*       must be saved and restored.                                      */
    if(ubADCCalibrationPerformed == 0)
    {
      /* Run ADC self calibration */
...
/**
  * @brief  Low-power mode initialization
  * @param  None
  * @retval None
  */
void LowPowerModeInit(void)
{
  ...
  /* Enable events capability to wake-up the CPU */
  LL_LPM_EnableEventOnPend();
}

/**
  * @brief  Acquisition initialization
  * @param  None
  * @retval None
  */
void AcquisitionInit(void)
...
/**
  ...
  * @note   Acquisition timer started, but ADC conversion must be started
  *         by SW at timer trigger event using "LL_ADC_REG_StartConversion()"
  *         (first ADC conversion not performed immediately).
  *         and start the timer triggering periodic ADC conversions
  * @param  None
  * @retval None
  */
void AcquisitionStart(void)
{
  /* Enable RTC timer triggering ADC conversions */
  LL_RTC_DisableWriteProtection(RTC);
...
void AcquisitionStop(void)
{
  /* Disable RTC timer triggering ADC conversions */
  LL_RTC_DisableWriteProtection(RTC);
  LL_RTC_WAKEUP_Disable(RTC);
  LL_RTC_EnableWriteProtection(RTC);
  ...
  if ((LL_ADC_IsEnabled(ADC1) == 1)               &&
      ...
      (LL_ADC_REG_IsConversionOngoing(ADC1) == 1)   )
  {
    LL_ADC_REG_StopConversion(ADC1);
  }

  /* Call the same function as ADC end of conversion interrupt callback */
  /* Acquisition resume after exiting from low-power mode */
  AcquisitionPauseLowPower();
...
/**
  * @brief  Acquisition pause before entering in low-power mode
  * @param  None
  * @retval None
  */
void AcquisitionPauseLowPower(void)
{
  ...
  /* Note: Request of wait for interrupt is done automatically                */
  /*       with CPU feature SLEEPONEXIT enabled                               */
  /*       using function "LL_LPM_EnableSleepOnExit()".                       */
  /*       (executing instructions WFI or WFE while interrupt or event        */
  /*        pending is not possible).                                         */
}

/**
  * @brief  Acquisition resume after exiting from low-power mode
  * @param  None
  * @retval None
  */
void AcquisitionResumeLowPower(void)
...
/**
  * @brief  De-Initialize LED3.
  * @param  None
  * @retval None
  */
void LED_DeInit(void)
{
  /* Configure IO in analog to reduce leakages */
  LL_GPIO_SetPinMode(LED3_GPIO_PORT, LED3_PIN, LL_GPIO_MODE_ANALOG);

  /* Disable the LED3 Clock */
...
/**
  * @brief  Set system clock for low-power execution: Decrease CPU frequency,
  *         adjust voltage scaling, disable unused system peripherals
  * @param  None
  * @retval None
  */
void SystemClock_Config_LowPower(void)
{
  ...
  /* ========== Perform MSI calibration from LSE ========== */
  /* Enable access to backup domain */
  LL_PWR_EnableBkUpAccess();

  /* Enable LSE */
  /* Note: Enable LSE if not already enabled previously */
  ...
  /* Note: Keep backup access enabled for more simple control of RTC afterwards */
  /* Disable access to backup domain */
  /* LL_PWR_DisableBkUpAccess(); */

  /* ========== Set system clock from MSI ========== */
  /* Set Frequency system clock source to MSI */
  ...
  /* Enable MSI Range Selection. */
  LL_RCC_MSI_EnableRangeSelection();

  /* Set MSI frequency */
  LL_RCC_MSI_SetRange(LL_RCC_MSIRANGE_6); /* MSI range 6: 4MHz */

  /* Adjust Flash Wait state after decrease Clock Frequency */
  LL_FLASH_SetLatency(LL_FLASH_LATENCY_0); /* With Vcore range 2, system frequency max 6MHz */

  /* Set Vcore voltage scaling to Range 2 (allowed for system frequency below 26MHz) */
  ...
  /* Note: Activation of mode low-power run is possible with this system      */
  /*       configuration but must not be done due to low-power mode Stop 2    */
  /*       requested afterwards in this example (transition of low-power      */
  /*       modes low-power run to Stop 2 is not possible).                    */
}

/**
  * @brief  Perform MSI calibration from LSE
  * @param  None
  * @retval None
  */
void SystemClock_MSI_calibration(void)
