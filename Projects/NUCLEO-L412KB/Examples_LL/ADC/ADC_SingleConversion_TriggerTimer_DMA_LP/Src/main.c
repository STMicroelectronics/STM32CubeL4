/**
  ******************************************************************************
  * @file    Examples_LL/ADC/ADC_SingleConversion_TriggerTimer_DMA_LP/Src/main.c
  ...
  *          to perform an acquisition at low frequency (between 1kHz and 100kHz).
  *          This examples uses low-power mode sleep.
...
/* Definitions of data related to this example */
  ...
  /* freq min 10Hz, range is min=10Hz, max=320kHz.                            */
  ...
  #define TIMER_FREQUENCY_RANGE_MIN      (  10U)
...
int main(void)
{
  ...
  /* Note: To start ADC conversion immediately without press on user button,  */
  /*       uncomment the line below.                                          */
  /* UserButton_Callback(); */

#if defined(OPTION_PWR_OPTIM_CPU_LOW_POWER_MODE_SLEEP)
  /* Configure and request CPU low power mode */

  /* Set CPU low-power mode sleep */
  LL_LPM_EnableSleep();
  ...
  __WFI();
#endif /* OPTION_PWR_OPTIM_CPU_LOW_POWER_MODE_SLEEP */
...
void Configure_TIM_TimeBase_ADC_trigger(void)
{
  ...
  NVIC_SetPriority(TIM2_IRQn, 0); /* TIM IRQ greater priority than DMA IRQ */
  ...
  /*## Configuration of TIM interruptions ####################################*/
  /* Enable interruption TIM time base update*/
  #if defined(OPTION_PWR_OPTIM_ADC_DISABLE)
...
void SystemClock_Config_LowPower(void)
{
  ...
  /* Set MSI frequency */
  LL_RCC_MSI_SetRange(LL_RCC_MSIRANGE_4); /* MSI range 4: 1MHz */
  ...
  /* Set Voltage scaling to Range 2. Decrease VCore  */
  LL_PWR_SetRegulVoltageScaling(LL_PWR_REGU_VOLTAGE_SCALE2);

  /* Activate low-power run mode */
...
void AdcGrpRegularUnitaryConvComplete_Callback()
{
  ...
  /* Check that ADC conversion is stopped before performing ADC de-activation */
  /* Note: ADC group regular conversion should be stopped at this step        */
  /*       because ADC continuous mode is disabled and trigger from timer     */
  /*       should let timing margin between 2 ADC conversion trigger events.  */
  /*       Check could be done using function                                 */
