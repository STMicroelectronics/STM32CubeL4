...
This application creates two threads.
   ...
   + An Rx thread that blocks on a queue to wait for data, blinking the LED4 each 
...
Every 500ms the MCU will come out of the low power state to turn the LED4 on,
then return to the low power state for 20ms before leaving the low power
state again to turn the LED4 off.  This will be observed as a fast blinking
on the LED4.
