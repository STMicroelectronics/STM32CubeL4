...
#define  ADC_CONVERTED_DATA_BUFFER_SIZE   ((uint32_t)    256)    /* Testing purposes */

/* Timer for ADC trigger parameters */
#define TIMER_FREQUENCY                ((uint32_t) 200)    /* Timer frequency (unit: Hz). With a timer 16 bits and time base freq min 1Hz, range is min=1Hz, max=32kHz. */
...
__IO uint16_t   aADCxConvertedData_master[ADC_CONVERTED_DATA_BUFFER_SIZE];       /* For the purpose of this example, dispatch dual conversion values into arrays corresponding to each ADC conversion values. */
__IO uint16_t   aADCxConvertedData_slave[ADC_CONVERTED_DATA_BUFFER_SIZE];       /* For the purpose of this example, dispatch dual conversion values into arrays corresponding to each ADC conversion values. */
...
static void ADC_Config(void)
{
  ...
  AdcHandle_master.Init.ExternalTrigConv      = ADC_EXTERNALTRIGCONV_Tx_TRGO;  /* Trig of conversion start done by external event (TIM2 here) */
  ...
  sConfig.Channel = ADCx_CHANNELa;
   sConfig.SamplingTime = ADC_SAMPLETIME_2CYCLES_5 /*ADC_SAMPLETIME_640CYCLES_5*/ /*ADC_SAMPLETIME_92CYCLES_5*/ /*ADC_SAMPLETIME_2CYCLES_5*/;
