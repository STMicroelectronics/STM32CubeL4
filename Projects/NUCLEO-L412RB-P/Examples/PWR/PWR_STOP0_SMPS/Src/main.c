...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* Range 2 set to 1.00V before disabling SMPS */
    HAL_PWREx_ExtSMSP_095_Disable();

    /********************************/
    /* 24 MHZ                       */
    /* PWR_REGULATOR_VOLTAGE_SCALE2 */
    /* Range 2 operating at 1.00 V  */
    /* SMPS IS ON                   */
    /********************************/

    /* Insert 1 millisecond delay */
    HAL_Delay(1);
...
static void Enter_SMPS_Mode(void)
{
  ...
  /* Check Ready Flag for switching to external SMPS */
  if (PWR_CR4_EXT_SMPS_ON != __HAL_PWR_GET_FLAG(PWR_FLAG_EXT_SMPS))
  {
    Error_Handler();
  }

  /* Range 2 can now be reduced to 0.95V */
  HAL_PWREx_EnableExtSMPS_0V95();

  /********************************/
  /* 24 MHZ                       */
  /* PWR_REGULATOR_VOLTAGE_SCALE2 */
  /* Range 2 operating at 0.95 V  */
  /* SMPS IS ON                   */
  /********************************/
