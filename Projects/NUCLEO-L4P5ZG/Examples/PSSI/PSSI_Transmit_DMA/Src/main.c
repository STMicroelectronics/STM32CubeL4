/**
  ******************************************************************************
  * @file    PSSI/PSSI_Transmit_DMA/Src/main.c
  * @author  MCD Application Team
  * @brief   This sample code shows how to use the STM32L4xx PSSI HAL API
  *          to transmit data in DMA mode.
  ******************************************************************************
  * @attention
  *
...
uint16_t pData16_16lines[]={0x0001,0x0002,0x0004,0x0008,0x0010,0x0020,0x0040,0x0080,0x0100,0x0200,0x0400,0x0800,0x1000,0x2000,0x4000,0x8000,
...
int main(void)
{
  ...
  /*##-1- Configure the PSSI peripheral #######################################*/
  hpssi.Instance = PSSI;
  ...
  /*##-2- Output MSI Clock / 16 on MCO pin to simulate external master clock.
  ...
  {
    /* Transmit operations */
    ValidationPSSI_HAL_PSSI_TransmitComplete_count = 0;
    if(HAL_OK != HAL_PSSI_Transmit_DMA(&hpssi, (uint32_t*)pData16, sizeof(pData16)))
    {
      Error_Handler();
    }

    while (ValidationPSSI_HAL_PSSI_TransmitComplete_count != 1) {};
    ...
    /* Transmit operations */
    ValidationPSSI_HAL_PSSI_TransmitComplete_count = 0;
    if(HAL_OK != HAL_PSSI_Transmit_DMA(&hpssi, (uint32_t*)pData16_16lines, sizeof(pData16_16lines)))
    {
      Error_Handler();
    }
...
{
  ValidationPSSI_HAL_PSSI_TransmitComplete_count++;
}
