/**
  ...
  *          =================================================================== 
  *          Notes:
  *           - This driver is intended for STM32L4R9I families devices only. 
  *           - The I2C EEPROM memory (M24LR64) is available on separate daughter 
  *             board ANT7-M24LR-A, which is provided with the STM32L4R9I-EVAL board.
  *             To use this driver with M24LR64, you have to connect
  *             the ANT7-M24LR-A to CN2 connector of STM32L4R9I-EVAL board.
  *          ===================================================================
  ******************************************************************************
  * @attention
  *
...
#define BUFFER_SIZE1            (countof(Tx1Buffer)-1 + 18)
...
static void EEPROM_SetHint(void)
{
  ...
  BSP_LCD_DisplayStringAt(0, 45, (uint8_t*)"EEPROM Features, use Wakeup push-button", CENTER_MODE); 
...
static void EEPROM_Show_Feature(uint8_t feature)
{
  uint8_t Tx1Buffer[] = "STM32L4R9I EEPROM";
  uint8_t EEPROM_I2C[] = "I2C"; 
  uint8_t *Bus = NULL;
  
  uint8_t Rx1Buffer[BUFFER_SIZE1+1] = {0}; 
  ...
  case 0:
    ...
    if (EepromDetected == 1)
    {
      ...
      if (BSP_EEPROM_ReadBuffer(Rx1Buffer, EEPROM_READ_ADDRESS1, (uint16_t *)(&NumDataRead)) != EEPROM_OK)
      {
        ...
        BSP_LCD_SetTextColor(LCD_COLOR_BLACK);
        BSP_LCD_DisplayStringAt(0, 145, (uint8_t*)"Press again Wakeup push-button", CENTER_MODE); 
        BSP_LCD_DisplayStringAt(0, 160, (uint8_t*)"To write new data", CENTER_MODE); 
        return;
      }
      /* Insure end of string present for successful display */
      Rx1Buffer[BUFFER_SIZE1] = '\0';
      BSP_LCD_DisplayStringAt(0, 115, (uint8_t*)"String read in the current EEPROM selected:", CENTER_MODE); 
      BSP_LCD_SetTextColor(LCD_COLOR_BLUE);    
      BSP_LCD_DisplayStringAt(0, 130, Rx1Buffer, CENTER_MODE); 
      BSP_LCD_SetTextColor(LCD_COLOR_BLACK);
      BSP_LCD_DisplayStringAt(0, 145, (uint8_t*)"Press Wakeup push-button", CENTER_MODE); 
      BSP_LCD_DisplayStringAt(0, 160, (uint8_t*)"To write new data", CENTER_MODE); 
    }
    else
    {
  ...
  case 1:
    ...
    if (EepromDetected == 1)
    {
      ...
      {
        BSP_LCD_DisplayStringAt(0, 115, (uint8_t*)"String written in the current EEPROM selected:", CENTER_MODE); 
        BSP_LCD_SetTextColor(LCD_COLOR_BLUE);    
        BSP_LCD_DisplayStringAt(0, 130, Tx2Buffer, CENTER_MODE); 
      }
      else 
      {
        BSP_LCD_SetTextColor(LCD_COLOR_RED);    
      ...
      BSP_LCD_DisplayStringAt(0, 145, (uint8_t*)"Press Wakeup push-button to end test.", CENTER_MODE);
    }
    else
    {
      BSP_LCD_SetTextColor(LCD_COLOR_RED);    
