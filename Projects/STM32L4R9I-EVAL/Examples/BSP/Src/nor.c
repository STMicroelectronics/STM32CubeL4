...
void NOR_demo (void)
{
  ...
  /********************************** PART 1 **********************************/
  /* Disable the LCD to avoid the refresh from the NOR */
  BSP_LCD_DisplayOff();
  BSP_LCD_DeInit();
...
//  BSP_RGB_LCD_LayerRgb565Init(LTDC_DEFAULT_ACTIVE_LAYER, LCD_FB_START_ADDRESS);
  ...
  BSP_LCD_DisplayStringAt(20, 190, (uint8_t*)"Press Wakeup push-button to switch to next part", CENTER_MODE);
  BSP_LCD_DisplayStringAt(20, 205, (uint8_t*)"and wait for end of memory erase", CENTER_MODE);
  ...
  /*##-1- Erase NOR memory ###################################################*/
  if(BSP_NOR_Erase_Chip() != NOR_STATUS_OK)
  {
    ubEraseStatus++;
  }
  ...
  /* Write data to the NOR memory */
  if(BSP_NOR_ProgramData(WRITE_READ_ADDR, nor_aTxBuffer, BUFFER_SIZE) != NOR_STATUS_OK)
  {
    ubWriteStatus++;
  }
  ...
  if(ubWriteStatus != 0)
  {
    BSP_LCD_DisplayStringAt(20, 130, (uint8_t*)"NOR PROGRAM : FAILED.", LEFT_MODE);
    BSP_LCD_DisplayStringAt(20, 145, (uint8_t*)"NOR Test Aborted.", LEFT_MODE);
  }
  else
  {
    BSP_LCD_DisplayStringAt(20, 130, (uint8_t*)"NOR PROGRAM : OK.     ", LEFT_MODE);
  }
  if(ubReadStatus != 0)
  {
    BSP_LCD_DisplayStringAt(20, 145, (uint8_t*)"NOR READ : FAILED.", LEFT_MODE);
  ...
  BSP_LCD_DisplayStringAt(20, 190, (uint8_t*)"Press Wakeup push-button to switch to next menu", CENTER_MODE);
...
static void NOR_SetHint(void)
{
  ...
  BSP_LCD_DisplayStringAt(0, 60, (uint8_t*)"Use Wakeup push-button to start", CENTER_MODE);
