...
One EXTI line (EXTI13) is configured to generate an interrupt on each falling edge.

The MCU enters into low power mode after 2 seconds and recursively wakes up on EXTI interrupt
which toggles a LED before to enter again in low power mode.

In this example:
    - EXTI13 is connected to PC.13 pin
      - when falling edge is detected on EXTI13 by pressing Wakeup push-button, LED1 toggles once

On STM32L4R9I-EVAL:
    - EXTI13 is connected to Wakeup push-button
