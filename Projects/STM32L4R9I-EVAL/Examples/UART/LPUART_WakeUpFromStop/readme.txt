...
Board: STM32L4R9I-EVAL (embeds a STM32L4R9AI device)
Tx Pin: PB.7 (CN7.2)
Rx Pin: PB.8 (CN7.3)
...
just to wait for board 1 LED1 to be turned off then to press board 2 Wakeup push-button 
to send the first wake-up stimulus to board 1. 
This starts the test sequence and no more action is required from the user
(a single Wakeup push-button press is needed to start the full 4 wake-up events
test sequence).
...
clock (SYSCLK) to run at 120 MHz with MSI as source of main PLL.
Additionally the HSI is enabled to demonstrate LPUART wake-up capabilities.
Each time board 1 is awoken, the system clock (SYSCLK) at 120 MHz is restored.
...
@par Directory contents 
  ...
  - UART/LPUART_WakeUpFromStop/Inc/stm32l4xx_hal_conf.h    HAL configuration file
  - UART/LPUART_WakeUpFromStop/Inc/stm32l4xx_it.h          Interrupt handlers header file
  - UART/LPUART_WakeUpFromStop/Inc/main.h                  Header for main.c module  
  - UART/LPUART_WakeUpFromStop/Src/stm32l4xx_it.c          Interrupt handlers
  - UART/LPUART_WakeUpFromStop/Src/main.c                  Main program
  - UART/LPUART_WakeUpFromStop/Src/stm32l4xx_hal_msp.c     HAL MSP module
  - UART/LPUART_WakeUpFromStop/Src/system_stm32l4xx.c      STM32L4xx system source file
...
@par Hardware and Software environment 
  ...
  - This example has been tested with a couple of STM32L4R9I-EVAL boards embedding a 
    STM32L4R9AI device and can be easily tailored to any other supported device 
    and development board.
    
  - STM32L4R9I-EVAL set-up
    - Connect a wire between 1st board PB.7 (CN7.2) pin (LPUART Tx) and 2nd board PB.8 (CN7.3) pin (LPUART Rx)
    - Connect a wire between 1st board PB.8 (CN7.3) pin (LPUART Rx) and 2nd board PB.7 (CN7.2) pin (LPUART Tx)
