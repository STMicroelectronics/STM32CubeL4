/**
  ...
  * @brief   This sample code shows how to use the STM32L4R9xx WWDG HAL API
...
int main(void)
{
  ...
  /*  Configuration:
      ...
      Timing calculation:
      a) WWDG clock counter period (in ms) = (4096 * 8) / (PCLK1(120Mhz) / 1000)
                                           = 0,2731 ms 
      b) WWDG timeout (in ms) = (127 + 1) * 0,2731
                              ~= 34,96 ms
      => After refresh, WWDG will expires after 34,96 ms and generate reset if 
      counter is not reloaded.
      c) Time to enter inside window
      Window timeout (in ms) = (127 - 80 + 1) * 0,2731
                             = 13,11 ms */
