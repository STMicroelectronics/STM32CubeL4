...
The WWDG timeout is set, through counter value, to 34,96 ms. 
The refresh window is set in order to make user wait 14 ms after a wadchdog refresh, 
before writing again counter. Hence the WWDG counter is refreshed each (14 + 1) ms in the 
...
occurs by pressing the Wakeup push-button (PC.13), the corresponding interrupt is served.
