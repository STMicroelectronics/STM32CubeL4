...
{
  WM_HWIN hItem;
  static uint8_t        prev_usb_status = -1;

  if ((uint != APP_DISK_UNIT) || (prev_usb_status == status))
    return;

  prev_usb_status = status;
...
#if USE_USB_DISK == 1
    if(APP_DISK_UNIT == USB_DISK_UNIT)
      TEXT_SetText(hItem, "USB NOT ready");
    else
#endif /* USE_USB_DISK */
      TEXT_SetText(hItem, "mSD NOT ready");
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    if((module_prop[k_ModuleGetIndex(&audio_player_board)].in_use) ||
      (k_StorageGetStatus(APP_DISK_UNIT) == 0))
    { 
      ...
      if (k_StorageGetStatus(APP_DISK_UNIT) == 0)
      {
        hItem = WM_GetDialogItem(pMsg->hWin, ID_FILENAMECAPTION);
        TEXT_SetText(hItem, ">>>");   
        hItem = WM_GetDialogItem(pMsg->hWin, ID_FILENAME);
