...
static uint8_t APP_DISK_UNIT = MSD_DISK_UNIT; /* USB_DISK_UNIT */

/* Private macros ------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { WINDOW_CreateIndirect,    "Audio Player", ID_FRAMEWIN_INFO,     0, 0, 480, 272, 0, 0x0, 0 },
...
static char const            *apDrives[NUM_DISK_UNITS];
...
static void _PlayFile(char *filename)
{
  ...
  if(haudio.out.state == AUDIOPLAYER_STOP)
  {
    if(AUDIOPLAYER_GetFileInfo(filename, &WavInfo) == 0)
    {
      ...
      {
        /* start playing */
        AUDIOPLAYER_Play(WavInfo.SampleRate, 1);
...
static void storage_update_status(uint8_t uint, uint8_t status)
{
  ...
  if ((uint != APP_DISK_UNIT) || (prev_storage_status == status))
    return;

  prev_storage_status = status;

  if(prev_storage_status == 0)
  {
  ...
  {
    char folder[128];
    snprintf(folder, sizeof(folder), "%s/Audio", k_Storage_GetDrive(uint));
    _AddEntireFolder((char *)k_Storage_GetDrive(uint), 0);
    _AddEntireFolder(folder, 0);
  ...
  case WM_NOTIFY_PARENT:
    ...
    case ID_EQUABAR_1: 
      if(NCode == WM_NOTIFICATION_VALUE_CHANGED)
      {
        ...
        AUDIOPLAYER_SetEqGain(0, 100 - SLIDER_GetValue(hItem));
      }
      break;      
    ...
    case ID_EQUABAR_2: 
      if(NCode == WM_NOTIFICATION_VALUE_CHANGED)
      {
        ...
        AUDIOPLAYER_SetEqGain(1, 100 - SLIDER_GetValue(hItem));
      }
      break;    
    ...
    case ID_EQUABAR_3: 
      if(NCode == WM_NOTIFICATION_VALUE_CHANGED)
      {
        ...
        AUDIOPLAYER_SetEqGain(2, 100- SLIDER_GetValue(hItem));
      }
      break;    
    ...
    case ID_EQUABAR_4: 
      if(NCode == WM_NOTIFICATION_VALUE_CHANGED)
      {
        ...
        AUDIOPLAYER_SetEqGain(3, 100- SLIDER_GetValue(hItem));
      }
      break;    
    ...
    case ID_EQUABAR_5: 
      if(NCode == WM_NOTIFICATION_VALUE_CHANGED)
      {
        ...
        AUDIOPLAYER_SetEqGain(4, 100- SLIDER_GetValue(hItem));
      }
      break;          
    ...
    case ID_EXIT_EQUAL_BUTTON:
      if(NCode == WM_NOTIFICATION_RELEASED)
      {
        osDelay(100);
        hItem = WM_GetDialogItem(pMsg->hWin, ID_LOUDBAR_1);
        k_BkupSaveParameter(CALIBRATION_AUDIOPLAYER_LOUD_BKP, 100- SLIDER_GetValue(hItem));
...
static void _cbMainDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_DELETE:
    ...
    /* DeInitialize Storage Units */
    k_StorageRegisterStatusCallBack(APP_DISK_UNIT, 0);
    k_StorageDeInit();
  ...
  case WM_NOTIFY_PARENT:
      ...
      {        
        if(AUDIOPLAYER_GetState() == AUDIOPLAYER_STOP)
        {
          ...
          else
          {
            ...
            hItem = ST_CHOOSEFILE_Create(pMsg->hWin,  60, 40, 360, 200, apDrives, GUI_COUNTOF(apDrives), 0, "Open an audio file", 0, &FileInfo); 
            WM_MakeModal(hItem);
        ...
        else if ((AUDIOPLAYER_GetState() == AUDIOPLAYER_PLAY) || (AUDIOPLAYER_GetState() == AUDIOPLAYER_PAUSE))
        {
          if(PlayerSettings.b.pause == PAUSE_ACTIVE)
          {   
            PlayerSettings.b.pause = PAUSE_INACTIVE;
    ...
    case ID_NEXT_BUTTON: 
      if(NCode == WM_NOTIFICATION_RELEASED)
      {
        ...
        {        
          ...
          if ((AUDIOPLAYER_GetState() == AUDIOPLAYER_PLAY) || (AUDIOPLAYER_GetState() == AUDIOPLAYER_PAUSE))
          {    
            if(PlayerSettings.b.pause == PAUSE_ACTIVE)
            {  
              PlayerSettings.b.pause = PAUSE_INACTIVE;
    ...
    case ID_WAV_BUTTON: 
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        hItem = ST_CHOOSEFILE_Create(pMsg->hWin,  60, 40, 360, 200, apDrives, GUI_COUNTOF(apDrives), 0, "Add a folder", 0, &FileInfo);        
        WM_MakeModal(hItem);
        r = GUI_WaitForDialogExec(hItem);
        if (r == 0) 
        {
    ...
    case ID_ADD_BUTTON: 
      ...
      case WM_NOTIFICATION_RELEASED:
        FileInfo.pfGetData = k_GetData;
        FileInfo.pMask = acMask_audio;
        hItem = ST_CHOOSEFILE_Create(pMsg->hWin,  60, 40, 360, 200, apDrives, GUI_COUNTOF(apDrives), 0, "Add to playlist", 0, &FileInfo);
        WM_MakeModal(hItem);
        r = GUI_WaitForDialogExec(hItem);
        if (r == 0) 
        {
...
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos)
{
  ...
  if(module_prop[k_ModuleGetIndex(&audio_player_board)].win_state == 0)
  {
    ...
    {
      if (k_StorageGetStatus(i))
      {
        apDrives[i] = k_Storage_GetDrive(i);
        APP_DISK_UNIT = i;
      }
    }

    if(k_StorageRegisterStatusCallBack(APP_DISK_UNIT, storage_update_status) == 0)
    ...
    AUDIOPLAYER_Init(PlayerSettings.b.volume, 1, 1);
    ...
    if (k_StorageGetStatus(APP_DISK_UNIT))
      storage_update_status(APP_DISK_UNIT, 1);
    else
      storage_update_status(APP_DISK_UNIT, 0);
