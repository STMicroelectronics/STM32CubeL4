     ...
     {            
       ...
       if(AUDIOPLAYER_SelectFile(input_file) == 0)
       {
        ...
        if (hItemRemainingDuration != 0)
        {
          ...
          sprintf(duration, "%02lu:%02lu", duration_sec / 60, duration_sec % 60);
...
static void _cbAudioWindow(WM_MESSAGE * pMsg) {
  ...
  case WM_TIMER:
    ...
    if(AUDIOPLAYER_GetState() == AUDIOPLAYER_PLAY)
    {
      ...
      if (hItemCurrentDuration != 0)
      {
        sprintf(tmp, "%02lu:%02lu", current_duration / 60, current_duration % 60);
      ...
      if (hItemRemainingDuration != 0)
      {
        sprintf(tmp, "%02lu:%02lu", remaining_duration / 60, remaining_duration % 60);
...
static void _cbAudioPlayer(WM_MESSAGE * pMsg) {
  ...
  case WM_CREATE:
    ...
    case WM_PAINT:
      ...
      {
        /* TODO : Show initialization message */
      }
