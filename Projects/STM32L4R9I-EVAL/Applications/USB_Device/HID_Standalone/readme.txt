...
This is a typical example on how to use the STM32L4R9I USB OTG Device peripheral where the STM32 MCU is
enumerated as a HID device using the native PC Host HID driver to which the STM32L4R9I-EVAL
board is connected. The Wakeup button is used to emulate the Mouse directions. 
When pressing the Wakeup button the cursor is moving 
...
to the active condition), and the Wakeup button is used as the remote wakeup source. 
