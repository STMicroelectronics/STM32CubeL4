/**
  ******************************************************************************
  * @file    DSI/DSI_ULPM_Data/Src/main.c
...
int main(void)
{
  ...
  /* Initialize Joystick, Joystick Sel push-button will be used to trigger ULPM Entry/Exit */
  ...
  while(1)
  {
    ...
    if (CheckForUserInput() > 0)
    {
      ...
      if(HAL_DSI_EnterULPMData(&DsiHandle) != HAL_OK)
      {
        Error_Handler();
      }
      BSP_LED_Off(LED2);
      ...
      if(HAL_DSI_ExitULPMData(&DsiHandle) != HAL_OK)
      {
        Error_Handler();
      }
      BSP_LED_On(LED2);
